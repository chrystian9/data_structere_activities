...
void lista::insertMiddle(const int valor){
	...
	if(auxPtr != NULL){
		...
		if(newNoh->next != NULL) newNoh->next->previous = newNoh;
		auxPtr->next = newNoh;
		size++;
...
noh* lista::acessaPosicao(){
	...
	while(aux != pos-1 and auxPtr != NULL){
		auxPtr = auxPtr->next;
		aux++;
	}
...
void lista::trocaPosicao(unsigned pos1, unsigned pos2){
	...
	cout << noh1->valor << " " << noh2->valor << endl;


	if(noh1==NULL or noh2==NULL){
		if(noh1 == NULL) cout << "Posicao 1 não existe" << endl;
		if(noh2 == NULL) cout << "Posicao 2 não existe" << endl;
	}else if(noh1->next == noh2){
		noh1->previous->next = noh2;
		noh1->next = noh2->next;
		noh2->next->previous = noh1;
		noh2->previous = noh1->previous;
		noh1->previous = noh2;
		noh2->next = noh1;
	}else if(noh2->next == noh1){
		noh2->previous->next = noh1;
		noh2->next = noh1->next;
		noh1->next->previous = noh2;
		noh1->previous = noh2->previous;
		noh2->previous = noh1;
		noh1->next = noh2;
	}else{
		if(noh1 == begin or noh1 == end or noh2 == begin or noh2 == end){
			if(begin == noh1) begin = noh2;
			if(begin == noh2) begin = noh1;
			if(end == noh1) end = noh2;
			if(end == noh2) end = noh1;
		}
		if(noh1->next != NULL) noh1->next->previous = noh2;
		if(noh1->previous != NULL) noh1->previous->next = noh2;
		
		if(noh2->next != NULL) noh2->next->previous = noh1;
		if(noh2->previous != NULL) noh2->previous->next = noh1;
		
		noh1->next = noh2->next;
		noh1->previous = noh2->previous;
		
		noh2->next = noh2->next;
		noh2->previous = noh2->previous;
	}
}
	...
	while(auxPtr != NULL){
		while(auxPtr2 != NULL){
			if(auxPtr2->valor == auxPtr->valor){
				removido = auxPtr2;
				auxPtr2->previous->next = auxPtr2->next;
				auxPtr2->next->previous = auxPtr2->previous;
